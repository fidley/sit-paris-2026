# We have agents, where are skills? 

This calls that we did before could be in fact create as skills and not agents. Let's then deep dive into topic of skills
and how they work with agents.

## No need to  sweat
``` There's a plenty of skills available online ```

If you go to [Skills.sh](https://skills.sh) you'll find lots of skill ready to download and use in your dev environment.
You'll find there also ARC-1 skills which you can easily download from git [Repo](https://github.com/arc-mcp/arc-1/tree/main/skills). Easiest way is to download full repo as zip
and then copy the full content of the skills folders into your special project under .github/skills folder.

Open the downloaded repo ZIP file and find Skills folder. Copy it.

<img width="1025" height="463" alt="image" src="https://github.com/user-attachments/assets/bc8b6edd-7df3-4924-9bba-c182a461c44c" />

Then locate the folder of the GHCopilotInstruction project, then paste the Skills folder into .github/ folder. 

<img width="744" height="224" alt="image" src="https://github.com/user-attachments/assets/03025dec-a629-4285-9393-9b0874481f62" />

In the Project Explorer refresh the content of the GHCopilotInstruction folder. Then in the Copilot Chat start writing `/skill`

<img width="1893" height="851" alt="image" src="https://github.com/user-attachments/assets/b2c9cf98-3eaf-479c-a6e2-2ded577b730c" />

As you can see, all skills are available to be called explicitly. Try one or more skill on any development object you have on your dev system.
For example ```/skill:explain-abap-code```. Check the results and asses their value.

## Go into skills interior



